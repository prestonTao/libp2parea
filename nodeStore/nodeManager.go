...
func Init(fileAbsPath, addrPre, pwd string) int {
	config.Wallet_keystore_default_pwd = pwd
	//加载本地网络id
	idinfo, err := BuildIdinfo(fileAbsPath, addrPre, config.Wallet_keystore_default_pwd)
	if err != nil {
		fmt.Println(err)
		return 1
	}
...
/*
	加载本地私钥生成idinfo
*/
func BuildIdinfo(fileAbsPath, addrPre, pwd string) (*IdInfo, error) {
	// fileAbsPath := filepath.Join(config.Path_configDir, config.Core_keystore)

	config.AddrPre = addrPre
	...
	if exist {
		err := keystore.Load(fileAbsPath, config.AddrPre)
		if err != nil {
			return nil, err
		}
	} else {
		err = keystore.CreateKeystore(fileAbsPath, config.AddrPre, pwd)
		if err != nil {
			return nil, err
		}
	}
...
func InitNodeStore(fileAbsPath, addrPre, pwd string) int {
	return Init(fileAbsPath, addrPre, pwd)
	// once.Do(run)
}
